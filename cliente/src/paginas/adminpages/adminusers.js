...
const AdminUsers = () => {
  ...
  const handleDeleteUser = async (userId) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este usuario?')) {
      try {
        await axios.delete(`http://localhost:4000/api/adminuser/users/${userId}`);
  ...
  const handleRegisterAdmin = async () => {
    try {
      await axios.post('http://localhost:4000/api/adminuser/register', newAdminData);
  ...
  useEffect(() => {
    ...
    const fetchUserMetrics = async () => {
      try {
        const response = await axios.get('http://localhost:4000/api/adminuser/users');
        setUserMetrics(response.data);
      } catch (error) {
        console.error('Error fetching user metrics:', error);
      }
...
const AdminUsers = () => {
  ...
  const handleRegisterAdmin = async () => {
    try {
      await axios.post('http://localhost:4000/api/adminuser/register', {
