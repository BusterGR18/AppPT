...
const AdminTelemetry = () => {
  ...
  useEffect(() => {
    const fetchUsersAndBoards = async () => {
      try {
        setIsLoading(true);
        const usersResponse = await axios.get(`${process.env.REACT_APP_API_URL}/api/adminuser/clients`);
        setUsers(usersResponse.data);
        setFilteredUsers(usersResponse.data);

        const boardPromises = usersResponse.data.map(user =>
          axios.get(`${process.env.REACT_APP_API_URL}/api/telemetry/boardIDs/${user.email}`)
        );
